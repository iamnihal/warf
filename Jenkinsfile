        ...
        always {
            script {
                if (readFile("Gitleaks_Output.json").length() > 3) {
                    emailext (to: 'nihalchoudhary55@gmail.com', attachmentsPattern: 'Gitleaks_Output.json', subject: "[Alert] Secret Found in Job - '${env.JOB_NAME}' | Build - '${env.BUILD_NUMBER}' ", body: 'Hi Team! Secret Scanner found a secret in recent commit, Please check attached file for more information.', mimeType: 'text/html');
                }else {
